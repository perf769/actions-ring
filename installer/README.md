...
Проверка обновлений доступна на вкладке «О программе». Автоматическая проверка и
фоновая загрузка управляются отдельными переключателями; фоновая загрузка доступна
при включённой автопроверке. Установка всегда требует подтверждения в окне с описанием
изменений. Загруженный архив сверяется с контрольной суммой SHA-256 из
официального релиза GitHub, после чего приложение закрывается, обновляет
установку текущего пользователя и запускается снова.

Если обновление подтверждено из портативной копии, новая версия устанавливается
в `%LOCALAPPDATA%\Programs\ActionsRing`; исходная портативная копия не удаляется.
